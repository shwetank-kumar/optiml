SELECT DISTINCT cost.WAREHOUSE_NAME, cost.WAREHOUSE_GROUP_NAME, cost.MEASURE_TYPE, SUM(cost.CREDITS_USED) as credits_used, sum(SUM(credits_used)) OVER (order by cost.START_TIME ASC) as Cumulative_Credits_Total, TIMESTAMPDIFF('hour', cost.START_TIME, cost.END_TIME) as duration, cost.START_TIME, cost.END_TIME  from (
SELECT DISTINCT
...
) as COST group by 7, 1, 2, 3, 6, 8 order by 7 asc
;
