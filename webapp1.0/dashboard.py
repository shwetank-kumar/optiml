...
def plot_total_usage_df(total_usage_df):
    ...
    df_prev = st.session_state.cqlib.total_cost_breakdown_ts(p1_sdate, p1_edate)
...
def plot_user_df(user_df):
    ...
    ## Get usage for previous week as a predictive sanity check
    df_prev = st.session_state.cqlib.cost_by_user_ts(st.session_state.p1_sdate, st.session_state.p1_edate)
...
def plot_warehouse_df(wh_df):
    ...
    ## Get usage for previous week as a predictive sanity check
    df_prev = st.session_state.cqlib.cost_by_wh_ts(st.session_state.p1_sdate, st.session_state.p1_edate)
...
def plot_partner_tool_df(partner_tool_df):
    ## Get usage for the past week
    df = st.session_state.cqlib.cost_by_partner_tool_ts(st.session_state.sdate, st.session_state.edate)
    ...
    ## Get usage for previous week as a predictive sanity check
    df_prev = st.session_state.cqlib.cost_by_partner_tool_ts(st.session_state.p1_sdate, st.session_state.p1_edate)
...
def get_resource_monitor_values(ts, te):
    print("Ts te: ", ts, te)
    df_compute = st.session_state.cqlib.cost_of_compute_ts(ts, te)
...
def query_dashboard(**kwargs):
    ...
    metric = 'credits'
    df_inefficient_queries = st.session_state.qqlib.n_inefficient_queries(st.session_state.sdate,
